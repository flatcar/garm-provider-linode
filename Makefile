...
GARM_PROVIDER_NAME := garm-provider-linode
...
fmtcheck:
	@gofmt -l -s $$(go list ./... | sed -n 's/github.com\/flatcar\/'$(GARM_PROVIDER_NAME)'\/\(.*\)/\1/p') | grep ".*\.go"; if [ "$$?" -eq 0 ]; then echo "gofmt check failed; please tun gofmt -w -s"; exit 1;fi
